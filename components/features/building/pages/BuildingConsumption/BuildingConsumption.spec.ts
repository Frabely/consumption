...
describe("BuildingConsumption logic", () => {
    ...
    it("handles reload flow, add floor action and short floor press", async () => {
        ...
        const loadHouses = vi.fn().mockResolvedValue([
            {id: "house-1", name: "House A", flats: [{id: "flat-1", name: "Floor 1", position: 0, rooms: []}]}
        ]);
        ...
        vi.doMock("@/store/hooks", () => {
            const values = [
                {name: "House A", flats: [{id: "flat-1", name: "Floor 1", position: 0, rooms: []}]},
        ...
        vi.doMock("@/components/features/building/pages/BuildingConsumption/BuildingConsumption.logic", async () => {
            const actual = await vi.importActual<typeof import("./BuildingConsumption.logic")>("./BuildingConsumption.logic");
            return {
                ...actual,
                canAccessBuildingConsumption: () => true,
                resolveFloorModalState: (isLongPress: boolean) => isLongPress ? ModalState.ChangeFloorFields : ModalState.AddFloorData
            };
        });
        ...
        vi.doMock("@/store/reducer/currentHouse", () => ({setCurrentHouse: (payload: unknown) => ({type: "setCurrentHouse", payload})}));
        vi.doMock("@/store/reducer/modalState", () => ({setModalState: (payload: ModalState) => ({type: "setModalState", payload})}));
        ...
        expect(dispatch).toHaveBeenCalledWith({type: "setIsReloadHousesNeeded", payload: false});
        expect(setHouseNames).toHaveBeenCalled();

        const menu = findElement(element, (currentElement) => currentElement.type === MenuBuildingMock);
        menu?.props?.onAddFloor?.();
        expect(dispatch).toHaveBeenCalledWith({type: "setModalState", payload: ModalState.AddFloor});
        ...
        expect(setCurrentFlat).toHaveBeenCalledWith(expect.objectContaining({name: "Floor 1"}));
        expect(dispatch).toHaveBeenCalledWith({type: "setModalState", payload: ModalState.AddFloorData});
    });
});
